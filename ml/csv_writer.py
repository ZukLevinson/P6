...
class CSVWriter:
    ...
    def create_folder(self):
        ...
        if os.listdir("data"):
            last_name = sorted(os.listdir(self.ROOT_FOLDER))[-1]
            new_id = int(last_name.split("_")[-1]) + 1

        new_dir_name = self.folder_format(new_id)
        os.mkdir(f"{self.ROOT_FOLDER}/{new_dir_name}")
    ...
    def create_csv_file(self, folder_name, columns=None):
        ...
        files = os.listdir(f"{self.ROOT_FOLDER}/{folder_name}")
        ...
        with open(f"{self.ROOT_FOLDER}/{folder_name}/{new_csv_name}.csv", "w") as file:
    ...
    def add_rows(self, path, rows):
        with io.open(f"{self.ROOT_FOLDER}/{path}", "a", encoding="utf-8", newline='') as file:
            writer = csv.writer(file)
            writer.writerows(rows)

    def count_rows(self, path):
        with io.open(f"{self.ROOT_FOLDER}/{path}", "r", encoding="utf-8") as file:
            writer = csv.reader(file)
            return len(list(writer))

    def count_csv_in_folder(self, path):
        return len(os.listdir(f"{self.ROOT_FOLDER}/{path}"))
